...
{
    public abstract class AbstractFileListener : IStartable
    {
        ...
        private readonly ILog logger = LogManager.GetLogger(typeof(AbstractFileListener));
        ...
        public void Start()
        {
            if (haveStarted)
                return;
            logger.DebugFormat("Starting eml transport on: {0}", Endpoint);
        ...
        protected void PeekMessageOnBackgroundThread(object state)
        {
            using (var queue = Endpoint.InitalizeQueue())
                while (!shouldStop)
                    ...
                    catch (ThreadAbortException) { } // nothing much to do here, process is being killed or someone is trying to do something rude to us
                    catch (Exception e)
                    {
#if DEBUG
                        Debugger.Break();
        ...
        {
            try { message = queue.Peek(TimeOutForPeek); }
        ...
        {
            try { return messageSerializer.Deserialize(transportMessage.BodyStream); }
            catch (Exception e)
            {
                try
                {
                    ...
                    {
                        var information = new FileCurrentMessageInformation
                        {
                            FileMessage = transportMessage,
                            Queue = messageQueue,
