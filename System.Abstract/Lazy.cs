...
namespace System
{
	[Serializable, DebuggerDisplay("ThreadSafetyMode={Mode}, IsValueCreated={IsValueCreated}, IsValueFaulted={IsValueFaulted}, Value={ValueForDebugDisplay}"), DebuggerTypeProxy(typeof(LazyDebugView<>)), ComVisible(false), HostProtection(SecurityAction.LinkDemand, Synchronization = true, ExternalThreading = true)]
	public class Lazy<T>
	{
		...
		static Lazy()
		{
			PUBLICATION_ONLY_OR_ALREADY_INITIALIZED = (() => default(T));
		}
		...
		{
			_threadSafeObj = Lazy<T>.GetObjectFromMode(mode);
		}
		...
		private Boxed CreateValue()
		{
			...
			if (_valueFactory != null)
			{
				try
				{
					if ((mode != LazyThreadSafetyMode.PublicationOnly) && (_valueFactory == Lazy<T>.PUBLICATION_ONLY_OR_ALREADY_INITIALIZED))
					...
					if (mode != LazyThreadSafetyMode.PublicationOnly)
						_valueFactory = Lazy<T>.PUBLICATION_ONLY_OR_ALREADY_INITIALIZED;
		...
		internal LazyThreadSafetyMode Mode
		{
			get
			{
				...
				if (_threadSafeObj == Lazy<T>.PUBLICATION_ONLY_OR_ALREADY_INITIALIZED)
